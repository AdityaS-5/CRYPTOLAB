    ...
    for bi, blk in enumerate(blocks):
        ...
        if operation.upper() == 'ENCRYPT':
            ...
            for r in range(1,10):
                ...
                steps.append('After SubBytes: ' + format_state_hex(state))
                state = shift_rows(state)
                steps.append('After ShiftRows: ' + format_state_hex(state))
                state = mix_columns(state)
                steps.append('After MixColumns: ' + format_state_hex(state))
                state = add_round_key(state, round_keys[r])
                steps.append('After AddRoundKey: ' + format_state_hex(state))
                steps.append('Round key: ' + round_keys[r].hex())
            ...
            for r in range(9,0,-1):
                ...
                steps.append('After InvShiftRows: ' + format_state_hex(state))
                state = inv_sub_bytes(state)
                steps.append('After InvSubBytes: ' + format_state_hex(state))
                state = add_round_key(state, round_keys[r])
                steps.append('After AddRoundKey: ' + format_state_hex(state))
            ...
            # For CBC decryption, XOR with prev_ct (which is IV for first block)
            if mode.upper() == 'CBC':
                ...
                prev_ct = blk  # blk is the ciphertext block for next iteration
            else:
                cipher_blocks.append(state)
                prev_ct = blk
